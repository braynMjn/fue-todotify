    ...
    <transition-group name="fade">
        <v-card class="mx-auto pa-3 ma-2 text-center" max-width="400" v-for="ToDo in ToDos" :key="ToDo.id" >
            <v-row class="d-flex justify-space-around">
                <v-cols cols="8"><v-list-item-title class="headline mb1">{{ ToDo.name }}</v-list-item-title></v-cols>
                <v-cols cols="4"><v-btn icon color="red" @click="deleteToDo(ToDo.id)">
                    <v-icon>mdi-delete</v-icon></v-btn></v-cols>
            </v-row>
        </v-card>
    </transition-group>
  </div>
